...
def build_user_prompt(prompt, solution):
...
def send(prompt, completion,):
    chat = lms.Chat(SYSTEM_PROMPT)
    chat.add_user_message(build_user_prompt(prompt, completion))
...
def compose_augmented_completion(reasoning, solution):
    """
...
def process_split(input_path,output_path):
    ...
    for idx, row in enumerate(rows, start=1):
        try:
            reasoning = send(prompt=row["prompt"], completion=row["completion"])
            
            augmented = compose_augmented_completion(reasoning, row["completion"])
...
if __name__ == "__main__":
    input_train = "datasets/current_to_run/train.jsonl"
    input_valid = "datasets/current_to_run/valid.jsonl"
    output_train = "datasets/current_to_run/train_reasoning.jsonl"
    output_valid = "datasets/current_to_run/valid_reasoning.jsonl"
    ...
    process_split(valid_in,valid_out)
